...
fs:
  ...
  - name: sqlx.toml
    ...
    node: false
  - name: migrations/
    role: timestamp-versioned forward-only SQL (38 files, latest adds account_member alias), embedded and run on boot and by every test template
    node: false
  - name: queries/
    role: one .sql file per statement, grouped by namespace directory
    node: true
  - name: src/
    role: port impls — uow (PgDatabase/PgUnitOfWork), account, user, commission, commission_changelog, actor_identity, profile, session_store, file_store, key_store, key_vault, plc_operation_log; @generated queries.rs; lib.rs pool/migrator
    node: false
  - name: tests/it/
    role: one integration binary sharing a Postgres container across ~20 modules — per-surface suites + the codegen_current, no_bare_pool_writes, single_binary_guard gates; account.rs updated for alias/list_for_user
    node: false
---
