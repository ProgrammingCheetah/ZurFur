...
use domain::entity_feed::{EntityFeedError, EntityFeedRepository};
...
async fn attach_feed_to_entity(pool: PgPool) {
    ...
    let ef = repo.attach(feed.id, EntityKind::Org, org.id).await.unwrap();

    assert_eq!(ef.feed_id, feed.id);
    assert_eq!(ef.entity_id, org.id);
    assert!(matches!(ef.entity_type, EntityKind::Org));
}
...
async fn list_by_entity(pool: PgPool) {
    ...
    let feeds = repo.list_by_entity(EntityKind::Org, org.id).await.unwrap();
...
async fn attach_already_attached_fails(pool: PgPool) {
    ...
    let result = repo.attach(feed.id, EntityKind::Org, org.id).await;
